...
var propTypes = {
  workbookName: _react.PropTypes.string,
  sheetInfo: _react.PropTypes.object,
  dashboardObjects: _react.PropTypes.object,
  filters: _react.PropTypes.object,
  url: _react.PropTypes.string,
  parameters: _react.PropTypes.object,
  parameterInfo: _react.PropTypes.object,
...
var TableauReport = function (_React$Component) {
  ...
  function TableauReport(props) {
    ...
    _this.state = {
      ...
      dashboardObjects: props.dashboardObjects,
      filters: props.filters,
      parameters: props.parameters,
      parameterInfo: props.parameterInfo,
    };
    return _this;
  }
  ...
  _createClass(TableauReport, [{
    ...
    value: function onComplete(promises, cb) {
      _es6Promise.Promise.all(promises).then(function(a) {
        return cb(a);
      }, function (a) {
        return cb(a);
      });
    ...
    key: 'getWorkbookName',
    value: function getWorkbookName(workbook) {
      var _this7 = this;
      ...
      return _this7.setState({ loading: false, workbookName: name });
    }
  }, { // added by demartsc
    key: 'getPublishedSheetsInfo',
    value: function getPublishedSheetsInfo(workbook) {
      var _this8 = this;
      ...
      promises.push(workbook.getPublishedSheetsInfo());

      this.onComplete(promises, function (a) {
        // console.log(a[0].length);
        for (var i = 0; i < a[0].length; i++) {
          //push all parm info to array based on js api 10.3
          infos.push({
              sheetName:a[0][i].getName(),
              sheetIndex: a[0][i].getIndex(), 
              sheetIsActive: a[0][i].getIsActive(),
              sheetIsHidden: a[0][i].getIsHidden(),
              sheetType: a[0][i].getSheetType(),
              sheetSize: a[0][i].getSize(),
          });
        }
      console.log(infos);
      return _this8.setState({ loading: false, parameterInfo: infos });
      });
    }
  }, { // added by demartsc
    key: 'getObjects',
    value: function getObjects(workbook) {
      var _this9 = this;
      ...
      promises.push(workbook.getActiveSheet().getObjects());
      ...
      this.onComplete(promises, function (a) {
        ...
        for (var i = 0; i < a[0].length; i++) {
          //push all parm info to array based on js api 10.3
          objects.push({
              //objectName: a[0][i].getName(), // object name is not getting returned by this
              objectType: a[0][i].getObjectType(),
              sheetObject: a[0][i].getWorksheet(),
              objectPosition: a[0][i].getPosition(),
              objectSize: a[0][i].getSize(), 
          });
        }
      console.log(objects);
      return _this9.setState({ loading: false, dashboardObjects: objects });
      });
    }
  }, {
    key: 'applyFilters',
    ...
    key: 'getParameters',
    value: function getParameters(workbook) {
      var _this6 = this;
      ...
      promises.push(workbook.getParametersAsync());
      ...
      this.onComplete(promises, function (a) {
        ...
        for (var i = 0; i < a[0].length; i++) {
          //push all parm info to array based on js api 10.3
          parms.push({
              parmName:a[0][i].getName(),
              parmDataType: a[0][i].getDataType(), 
              parmCurrentValue: a[0][i].getCurrentValue().value,
              parmCurrentFormattedValue: a[0][i].getCurrentValue().formattedValue,
              parmAllowableValuesType: a[0][i].getAllowableValuesType(), 
              parmAllowableValues: a[0][i].getAllowableValues(),
              parmMinValue: a[0][i].getMinValue(),
              parmMaxValue: a[0][i].getMaxValue(),
              parmStepSize: a[0][i].getStepSize(),
              parmDateStepPeriod: a[0][i].getDateStepPeriod(),
          });
        }
      console.log(parms);
      return _this6.setState({ loading: false, parameterInfo: parms });
      });
    }
  }, {
    key: 'applyParameters',
    ...
    key: 'initTableau',
    value: function initTableau(vizUrl) {
      var _this4 = this;

      var _props = this.props;
      var filters = _props.filters;
      var parameters = _props.parameters;
      var parameterInfo = _props.parameterInfo;

      //csd - modified this code to take new url if passed, otherwise use props
      var vizUrl = vizUrl || this.getUrl();
      ...
      var options = _extends({}, filters, parameters, this.props.options, {
        onFirstInteractive: function onFirstInteractive() {
          ...
          _this4.getWorkbookName(_this4.workbook);
          _this4.getPublishedSheetsInfo(_this4.workbook);
          _this4.getObjects(_this4.workbook);
          _this4.getParameters(_this4.workbook);
